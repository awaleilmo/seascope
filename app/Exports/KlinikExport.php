...
class KlinikExport implements FromQuery, WithHeadings, WithStyles, WithMapping, WithColumnWidths, WithDrawings
{
    ...
    public function query(): Relation|Builder|\Illuminate\Database\Query\Builder
    {
        return report_klinik::query()
            ->leftJoin('poldas', 'poldas.id', 'report_kliniks.polda_id')
            ...
            ->where('report_kliniks.polda_id', '=', $this->poldaId)
            ->where('report_kliniks.tanggal', '>=', $this->tglDari)
            ->where('report_kliniks.tanggal', '<=', $this->tglSampai);
    }
    ...
    {
        return 'Jumlah Pasien: '. $str1 . ' orang;' . "\n \n" . 'Keluhan penyakit pasien: '. $str2 . ';' . "\n \n" . 'Obat yang diberikan kepada pasien: '. $str3 . ';';
    }
    public function map($report_klinik): array
    {
        $personil = $this->sortPersonil(
            json_decode($report_klinik->personil_jml),
            json_decode($report_klinik->personil_sat)
        );
        $hasil = $this->sortHasil(
            $report_klinik->jml_peserta,
            $report_klinik->keluhan_peserta,
            $report_klinik->obat
        );
        return $report_klinik->num_row == 1 ? [
            [],
            [],
            [],
            [],
            ...
            [],
            [
                $report_klinik->num_row,
                $report_klinik->polda_name,
                $personil,
                $report_klinik->lokasi,
                '',
                $hasil,
                $report_klinik->keterangan,
            ]
        ] : [
            $report_klinik->num_row,
            $report_klinik->polda_name,
