...
class KiaController extends Controller
{
    ...
    public function page(Request $request): JsonResponse
    {
        try {
            ...
            $gakkum = Kia::where('no_lp', 'like', '%' . $data['search'] . '%');
    ...
    public function createUpdate(Request $request): JsonResponse
    {
        try {
            ...
            // Validation
            if ($data['id'] === null) {
                $check = Kia::where('no_lp', $data['no_lp'])->count();
                if ($check) {
                    return response()->json([
                        'status' => false,
                ...
                $gakkum = Kia::find($data['id']);
            }

            $subject = $data['id'] ? 'Update KIA' : 'Create KIA';
            ...
            $this->Logger()->logKia($subject, $gakkum->id, $method);
    ...
    public function destroy($id): JsonResponse
    {
        try {
            ...
            $this->Logger()->logKia('Delete Kia', $data, 'delete');
