...
// model
const Model = PoldaModel;
const ModelLokasi = LokasiModel;
...
const arrLokasi = ref([])
const columns = [
    {label: 'Nama', field: 'name', search: true, sortable: true, width: 'auto'},
    {label: 'Kota', field: 'kota', search: true, sortable: true, width: 'auto'},
    {label: 'Alamat', field: 'alamat', search: true, sortable: true, width: 'auto'},
    {label: 'Lokasi', field: 'lokasi_name', search: true, sortable: true, width: 'auto'},
...
const getLokasi = async () => {
    let rawLokasi = await ModelLokasi.getAll()
    let decode = DecodeConfig.base64toJSON(rawLokasi.data.content)
    arrLokasi.value = decode.map((item) => {
        return{
            id: item.id,
            name: item.name,
        }
...
const addFunction = async () => {
    ...
    popModal.value.title = 'Tambah Polda'
...
const editFunction = async (val) => {
    ...
    Model.lokasi_id = val.lokasi_id
    await getLokasi()
    popModal.value.status = true
    popModal.value.title = 'Ubah Polda'
    isLoading.value = false
}
...
const validation = () => {
    ...
    if (Model.kota === '') {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Kota Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    if (Model.alamat === '') {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Alamat Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    if (Model.lokasi_id === null) {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Lokasi Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    save();
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-4">
                            ...
                            <Select :model-value="Model.lokasi_id" @update:model-value="Model.lokasi_id = $event"
                                    :data="arrLokasi"/>
                        </div>
                    </div>
