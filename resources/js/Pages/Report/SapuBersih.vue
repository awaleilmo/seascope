...
import SapuModel from "@/Models/Sapu.model.js";
...
const columns = [
    ...
    {label: 'Sampah Organik', field: 'organik', search: true, sortable: true, width: 'auto'},
    {label: 'Sampah Anorganik', field: 'anorganik', search: true, sortable: true, width: 'auto'},
...
const resetModel = () => {
    ...
    Model.lokasi = '';
    Model.sampah_organik_jml = 0;
    Model.sampah_organik_ket = '';
    Model.sampah_anorganik_jml = 0;
    Model.sampah_anorganik_ket = '';
    Model.keterangan = '';
    Model.foto = null;
...
const addFunction = async () => {
    ...
    popModal.value.title = 'Tambah Data Sapu Bersih Sampah'
    isLoading.value = false
}
const editFunction = async (val) => {
    ...
    Model.personil_ket = JSON.parse(val.personil_ket)
    Model.personil_jml = JSON.parse(val.personil_jml)
    Model.personil_sat = JSON.parse(val.personil_sat)
    Model.lokasi = val.lokasi
    Model.sampah_organik_jml = val.sampah_organik_jml
    Model.sampah_organik_ket = val.sampah_organik_ket
    Model.sampah_anorganik_jml = val.sampah_anorganik_jml
    Model.sampah_anorganik_ket = val.sampah_anorganik_ket
    Model.keterangan = val.keterangan
    Model.foto = val.foto
    Model.tanggal = val.tanggal
    Model.waktu = val.waktu
    popModal.value.status = true
    popModal.value.title = 'Ubah Data Sapu Bersih Sampah'
    isLoading.value = false
}
...
const validation = async () => {
    ...
    if (Model.personil_ket.length === 0) {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Nama Personil Tidak Boleh Kosong'
    }
...
const save = async () => {
    ...
    Model.personil_ket = JSON.stringify(Model.personil_ket)
    isLoading.value = true
    rendering.value = false
    ...
    if (saveData.data.status) {
        ...
        Model.personil_ket = JSON.parse(Model.personil_ket)
        alerts.value.status = true
...
const addPersonil = () => {
    renderPersonil.value = false
    Model.personil_ket.push('')
    Model.personil_jml.push(0)
    Model.personil_sat.push('pers')
    renderPersonil.value = true
}
const removePersonil = (index) => {
    renderPersonil.value = false
    Model.personil_ket.splice(index, 1)
    Model.personil_jml.splice(index, 1)
    Model.personil_sat.splice(index, 1)
    renderPersonil.value = true
}
...
const formatPersonil = (ket, jml, sat) => {
    let jsonKet = JSON.parse(ket)
    let jsonJml = JSON.parse(jml)
    let jsonSat = JSON.parse(sat)
    let personil = ''
    let nm = 1;
    jsonKet.map((item, index) => {
        personil += `${nm}. ${item} ${jsonJml[index]} ${jsonSat[index]} </br>`
        nm++
    })
    return personil
}
...
<template>
    ...
    <AuthenticatedLayout :props-loading="isLoading">
        <template #header>
            Sapu Bersih Sampah Pantai & Laut
        </template>
                        ...
                        <template #table-row="props">
                                <span v-if="props.column.field === 'jumlah'">
                                  <span class="capitalize"
                                        v-html=" formatPersonil(props.row.personil_ket, props.row.personil_jml, props.row.personil_sat)"/>
                                </span>
                            <span v-if="props.column.field === 'organik'">
                                    <span class="capitalize">
                                        {{ props.row.sampah_organik_jml }} Kg <br> {{ props.row.sampah_organik_ket }}
                                    </span>
                                </span>
                            <span v-if="props.column.field === 'anorganik'">
                                    <span class="capitalize">
                                        {{ props.row.sampah_anorganik_jml }} Kg <br> {{
                                            props.row.sampah_anorganik_ket
                                        }}
                                    </span>
                                </span>
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-4">
                            ...
                            <button @click="addPersonil()" class="w-full bg-gray-200 rounded hover:bg-gray-300">
                                <font-awesome-icon icon="fa-solid fa-plus" class="mx-1"/>
                            </button>
                            <div class="rounded border px-2 py-3">
                                <div class="grid grid-cols-3 gap-2">
                                    <InputLabel class="px-2 font-bold" value="Nama Personil"/>
                                    <InputLabel class="px-2 font-bold" value="Jumlah Personil"/>
                                    <InputLabel class="px-2 font-bold" value="Personil"/>
                                </div>
                                <div v-if="renderPersonil" v-for="(personil, index) in Model.personil_ket"
                                     class="grid grid-cols-3 gap-2"
                                     :class="Model.personil_ket.length > 1 ? 'mr-10' : ''"
                                >
                                    <TextInput
                                        type="text"
                                        class="mt-1 block w-full capitalize"
                                        v-model="Model.personil_ket[index]"
                                    ...
                                    <TextInput
                                        ...
                                        placeholder="Personil ( Personil, Orang, Kompi )"
                                    />
                                    <danger-warning v-if="Model.personil_ket.length > 1" class="absolute right-4 mt-1"
                                                    @click="removePersonil(index)">
                        ...
                        <div class="mt-4">
                            <InputLabel value="Sampah Organik"/>
                            <div class="flex flex-row gap-2">
                                <TextInput
                                    type="number"
                                    class="mt-1 h-11 w-32 capitalize"
                                    v-model="Model.sampah_organik_jml"
                                    placeholder="Berat Kg"
                                />
                                <TextInputArea
                                    type="text"
                                    class="mt-1 block w-full capitalize"
                                    v-model="Model.sampah_organik_ket"
                                    placeholder="Keterangan Sampah Organik"
                                />
                            </div>
                        </div>
                        <hr class="mt-4"/>
                        <div class="mt-4">
                            <InputLabel value="Sampah Anorganik"/>
                            <div class="flex flex-row gap-2">
                                <TextInput
                                    type="number"
                                    class="mt-1 h-11 w-32 capitalize"
                                    v-model="Model.sampah_anorganik_jml"
                                    placeholder="Berat Kg"
                                />
                                <TextInputArea
                                    type="text"
                                    class="mt-1 block w-full capitalize"
                                    v-model="Model.sampah_anorganik_ket"
                                    placeholder="Keterangan Sampah Anorganik"
                                />
                            </div>
                        </div>
        ...
        <PrintPopUp
            ...
            title="Print Sapu Bersih Sampah Pantai & Laut"
            urlPrint="/api/report/sapu/download"
        />
