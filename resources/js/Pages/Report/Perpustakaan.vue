...
import PerpustakaanModel from "@/Models/Perpustakaan.model.js";
...
const columns = [
    ...
    {label: 'Jumlah Peserta', field: 'jml_peserta', search: true, sortable: true, width: 'auto'},
    {label: 'Asal Peserta', field: 'asal_peserta', search: true, sortable: true, width: 'auto'},
    {label: 'Hasil', field: 'hasil', search: true, sortable: true, width: 'auto'},
...
const addFunction = async () => {
    ...
    popModal.value.title = 'Tambah Data Laporan Perpustakaan'
...
const editFunction = async (val) => {
    ...
    Model.asal_peserta = val.asal_peserta
    Model.hasil = val.hasil
    Model.keterangan = val.keterangan
    Model.foto = val.foto
    Model.tanggal = val.tanggal
    Model.waktu = val.waktu
    popModal.value.status = true
    popModal.value.title = 'Ubah Data Laporan Perpustakaan'
    isLoading.value = false
}
...
const validation = async () => {
    ...
    if (Model.jml_peserta === '') {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Jumlah Peserta Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    if (Model.asal_peserta === '') {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Asal Peserta Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    if (Model.hasil === '') {
        modalHeader.scrollIntoView({behavior: 'smooth'})
        alerts.value.status = true
        alerts.value.message = 'Hasil Tidak Boleh Kosong'
        alerts.value.type = 'danger'
        return false;
    }
    if (Model.tanggal === '') {
...
<template>
    ...
    <AuthenticatedLayout :props-loading="isLoading">
            ...
            <div class="relative">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="p-6">
                        ...
                        <hr class="mt-4"/>
                        <div class="mt-4">
                            <InputLabel value="Asal Peserta"/>
                            <TextInputArea
                                class="mt-1 block w-full capitalize"
                                v-model="Model.asal_peserta"
                                placeholder="Asal Peserta ..."
                            />
                        </div>
                        <hr class="mt-4"/>
                        <div class="mt-4">
        ...
        <PrintPopUp
            ...
            title="Print Laporan Perpustakaan Terapung"
            urlPrint="/api/report/perpustakaan/download"
        />
