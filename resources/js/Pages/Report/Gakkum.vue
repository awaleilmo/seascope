...
import GakkumModel from "@/Models/Gakkum.model.js";
...
// model
const Model = reactive(GakkumModel);
...
// object
const title = ref('DATA PENEGAKKAN PERKARA SUBDIT GAKKUM')
const viewModal = ref({
    status: false,
    title: 'DATA PENEGAKKAN PERKARA SUBDIT GAKKUM',
})
const popModal = ref({
    status: false,
    title: '',
...
const printFn = () => {
    ...
    window.location.href = window.location.protocol + '//' + window.location.host + '/api/gakkum/download?data=' + encode
    isLoading.value = false;
}
...
<template>
    ...
    <AuthenticatedLayout :props-loading="isLoading">
        <template #header>
            DATA PENEGAKKAN PERKARA SUBDIT GAKKUM
        </template>
